...
function Home() {
  ...
  useEffect(() => {
    fetch("https://movie-vercel-back.vercel.app/movies")
  ...
  const movies = moviesData.map((data, i) => {
    ...
    return <Movie key={i} updateLikedMovies={updateLikedMovies} isLiked={isLiked} title={data.title} overview={data.overview.substring(0, 250)} poster={`https://image.tmdb.org/t/p/w600_and_h900_bestv2${data.poster_path}`} voteAverage={data.vote_average} voteCount={data.vote_count} />;
  });
